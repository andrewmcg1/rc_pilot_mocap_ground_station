...
/** SetTimeout
 * Requires:
 *      Must be called after Connect() and/or CreateUDPSocket()
 *      so that either 'mhSocket', 'mhUDPSocket', or 'mhUDPBroadcastSocket' are not -1
 * 
 * Inputs:
  *      microseconds    - Number of microseconds for the timeout
 *                        **Must be a positive integer**
 * 
 * Returns:
 *      TRUE    - if any existing sockets ('mhSocket', 'mhUDPSocket', or 'mhUDPBroadcastSocket')
 *                have their timeout set sucessfully
 * 
 *      FALSE   - if timeout cannot be set on existing sockets
 *                or there are no existing sockets
 *                or 'seconds' or 'microseconds' are negative
 */
bool CNetwork::SetTimeout(int microseconds)
{
    //Exit if time values are negative
    if(microseconds < 0 )
    {
        printf("ERROR in CNetwork::SetTimeout. Input must be non-negative\n");
        return false;
    }

    //Exit if none of the sockets are initialized (-1 is used for un-initialized socket)
    if( (mhSocket == -1) && (mhUDPSocket == -1) && (mhUDPBroadcastSocket == -1))
    {
        printf("ERROR in CNetwork::SetTimeout. No sockets initialized\n");
        return false;
    }
    ...
    //Set timeout for 'mhSocket' if it has been initialized
    if(mhSocket != -1)
    {
        if(setsockopt(mhSocket, SOL_SOCKET, SO_RCVTIMEO, (const char*) &timeout, sizeof(timeout)) == 0)
        {
            printf("Timeout on Socket 'mhSocket' set to %lf seconds\n", 
                (double) timeout.tv_sec + (double) timeout.tv_usec/1e6);
        }
        else
        {
            printf("Unable to set timeout for socket 'mhSocket'\n");
            return false;
        }
    }

    //Set timeout for 'mhUDPSocket' if it has been initialized
    if(mhUDPSocket != -1)
    {
        if(setsockopt(mhUDPSocket, SOL_SOCKET, SO_RCVTIMEO, (const char*) &timeout, sizeof(timeout)) == 0)
        {
            printf("Timeout on Socket 'mhUDPSocket' set to %lf seconds\n", 
                (double) timeout.tv_sec + (double) timeout.tv_usec/1e6);
        }
        else
        {
            printf("Unable to set timeout for socket 'mhUDPSocket'\n");
            return false;
        }
    }

    //Set timeout for 'mhUDPBroadcastSocket' if it has been initialized
    if(mhUDPBroadcastSocket != -1)
    {
        if(setsockopt(mhUDPBroadcastSocket, SOL_SOCKET, SO_RCVTIMEO, (const char*) &timeout, sizeof(timeout)) == 0)
        {
            printf("Timeout on Socket 'mhUDPBroadcastSocket' set to %lf seconds\n", 
                (double) timeout.tv_sec + (double) timeout.tv_usec/1e6);
        }
        else
        {
            printf("Unable to set timeout for socket 'mhUDPBroadcastSocket'\n");
            return false;
        }
    }
    return true;
...
int CNetwork::Receive(char* rtDataBuff, int nDataBufSize, bool bHeader, int nTimeout, unsigned int *ipAddr)
{
   ...
   timeval  timeout = {0, 0};
    ...
    else
    {
        ...
        timeout.tv_usec = nTimeout%1000000;//nTimeout % 1000000;
        pTimeout = &timeout;
    }
    //-------------------------------------------------------------------
   ...
   // int test[nDataBufSize]; 
    ...
    {
        //If you attempted to set a timeout for the socket, you'll get one of these error messages. 
        //In this case, return -1, but don't disconnect the socket. It's fine
        if(errno == EAGAIN || errno == EWOULDBLOCK)
        {
            // printf("Socket Timed out\n");
            return SOCKET_TIMEOUT_RETURN_VALUE;
        }
    }
    
    //If we get here, disconnect the socket and return -1
    SetErrorString();
    Disconnect();
    return -1;
} // RecvMessage
