...
CRTPacket* CRTProtocol::ReceiveRTPacket(CRTPacket::EPacketType &eType, bool bSkipEvents, int nTimeout)
{
    ...
    do 
    {
        ...
        //Haven't received anything within the timeout period. 
        //This isn't a problem, but we should skip everything else in this function
        if(nRecved == SOCKET_TIMEOUT_RETURN_VALUE)
        {
            return NULL;
        }
...
bool CRTProtocol::Read6DOFSettings()
{
            ...
            if (eType == CRTPacket::PacketXML)
            {
                ...
                if (oXML.FindChildElem("The_6D") && oXML.IntoElem())
                {
                    ...
                    if (oXML.FindChildElem("Bodies"))
                    {
                        ...
                        //Check for Euler angle order
                        if (oXML.FindChildElem("Euler"))
                        {
                            oXML.IntoElem(); //Make 'Euler' the current parent

                            //Get first rotation axis
                            if (oXML.FindChildElem("First"))
                            {
                                s6DBodySettings.eulerFirstAxis = oXML.GetChildData();
                            }
                            else
                            {
                                fprintf(stderr, "Failed to get First Euler axis\n");
                                bReturn = false;
                            }
                            ...
                            {
                                s6DBodySettings.eulerSecondAxis = oXML.GetChildData();
                            }
                            else
                            {
                                fprintf(stderr, "Failed to get Second Euler axis\n");
                                bReturn = false;
                            }
                            ...
                            {
                                s6DBodySettings.eulerThirdAxis = oXML.GetChildData();
                            }
                            else
                            {
                                fprintf(stderr, "Failed to get Third Euler axis\n");
                                bReturn = false;
                            }
                            oXML.OutOfElem(); // Leave 'Euler' and make 'Bodies' the parent
                            oXML.ResetChildPos(); //'Euler' comes after all the 'Body' sections. Reset so we can read each 'Body'
                        }
                        else
                        {
                            fprintf(stderr, "Failed to get 'Euler' child\n");
                            bReturn = false;
                        }// if/else FindChildElem("Euler")
                        ...
                        for (int iBody = 0; iBody < nBodies; iBody++)
                        {
                            if (oXML.FindChildElem("Body"))
                            {
                                ...
                                if (oXML.FindChildElem("Name"))
                                {
                                    ...
                                    if (oXML.FindChildElem("RGBColor"))
                                    {
                                        ...
                                        while (oXML.FindChildElem("Point"))
                                        {
                                            ...
                                            oXML.OutOfElem(); // Leave 'Point' and make 'Body' the parent
                                    ...
                                    {
                                        fprintf(stderr, "Failed to get 'RGBColor' of 'Body' %d of %d\n", iBody, nBodies);
                                        bReturn = false;
                                        break;
                                    }
                                }
                                ...
                                {
                                    fprintf(stderr, "Failed to get 'Name' of 'Body' %d of %d\n", iBody, nBodies);
                                    bReturn = false;
                                    break;
                                }
                                oXML.OutOfElem(); // Leave 'Body' and make 'Bodies' the parent
                            } //findChildElem("Body")
                            else
                            {
                                fprintf(stderr, "Failed to get 'Body' %d of %d\n", iBody, nBodies);
                                bReturn = false;
                                break;
                            }
                        }
    ...
    printf( "Read6DOFSettings returned %d\n", bReturn);
    return bReturn;
} // Read6DOFSettings
...
const char* CRTProtocol::Get6DOFBodyFirstEuler(unsigned int nBodyIndex)
{
    if (nBodyIndex < mvs6DOFBodySettings.size())
    {
        return mvs6DOFBodySettings[nBodyIndex].eulerFirstAxis.c_str();
    }
...
const char* CRTProtocol::Get6DOFBodySecondEuler(unsigned int nBodyIndex)
{
    if (nBodyIndex < mvs6DOFBodySettings.size())
    {
        return mvs6DOFBodySettings[nBodyIndex].eulerSecondAxis.c_str();
    }
...
const char* CRTProtocol::Get6DOFBodyThirdEuler(unsigned int nBodyIndex)
{
    if (nBodyIndex < mvs6DOFBodySettings.size())
    {
        return mvs6DOFBodySettings[nBodyIndex].eulerThirdAxis.c_str();
    }
